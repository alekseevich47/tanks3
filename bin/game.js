...
function startBattleEngine(mode = "ZACHISTKA") {
    ...
    skills.shield.readyAt = Date.now() + 10000;
    skills.shield.active = false;
    skills.rapid.currentMax = 10000;
...
function isCollidingMap(x, y, w, h) {
    ...
    for (let c of corners) {
        ...
        if (mapData[row] && mapData[row][col] && 
            (mapData[row][col].type === 1 || mapData[row][col].type === 2 || mapData[row][col].type === 3)) {
            return true;
        }
    }
    return false;
...
function fireBullet(isPlayer, sx, sy, sdir) {
    bullets.push({ x: sx, y: sy, w: 4, h: 4, speed: 5, dir: sdir, isPlayer: isPlayer });
}
...
function addTrack(t) {
    const now = Date.now();
    if (now - t.lastTrack > 200) {
        tracks.push({ x: t.x, y: t.y, dir: t.dir, time: now });
        t.lastTrack = now;
    }
}

function createBigExplosion(x, y) {
    explosions.push({ x: x, y: y, timer: 30, big: true });
}

function canSeePlayer(bot) {
    if (tank.isDead) return null;
    let dx = Math.abs(bot.x - tank.x);
    let dy = Math.abs(bot.y - tank.y);
    let alignX = dx < TILE - 4;
    let alignY = dy < TILE - 4;
    
    if (alignX) {
        let dir = tank.y < bot.y ? "up" : "down";
        let step = dir === "up" ? -TILE : TILE;
        let blocked = false;
        for (let y = bot.y; dir === "up" ? y > tank.y : y < tank.y; y += step) {
            let col = Math.floor((bot.x + bot.w/2) / TILE);
            let row = Math.floor((y + bot.h/2) / TILE);
            if (row < 0 || row >= mapData.length) break;
            if (mapData[row] && mapData[row][col] && mapData[row][col].type === 2) {
                blocked = true;
            }
        }
    ...
    if (alignY) {
        let dir = tank.x < bot.x ? "left" : "right";
        let step = dir === "left" ? -TILE : TILE;
        let blocked = false;
        for (let x = bot.x; dir === "left" ? x > tank.x : x < tank.x; x += step) {
            let col = Math.floor((x + bot.w/2) / TILE);
            let row = Math.floor((bot.y + bot.h/2) / TILE);
...
function moveTankGridState(t, isPlayer) {
    if (t.state === "idle") {
        let reqDir = null;
        if (isPlayer) {
            if (keys["ArrowUp"]) reqDir = "up";
            else if (keys["ArrowDown"]) reqDir = "down";
            else if (keys["ArrowLeft"]) reqDir = "left";
            else if (keys["ArrowRight"]) reqDir = "right";
        } else {
            reqDir = getSmartBotDirection(t);
        }
        if (reqDir) {
            if (t.dir !== reqDir) {
                t.dir = reqDir;
            ...
            let nextX = t.x;
            let nextY = t.y;
            if (t.dir === "up") nextY -= TILE;
            if (t.dir === "down") nextY += TILE;
            if (t.dir === "left") nextX -= TILE;
            if (t.dir === "right") nextX += TILE;
            if (!isCollidingMap(nextX, nextY, t.w, t.h) && 
                !checkTankCollisions(nextX, nextY, t, isPlayer)) {
                t.targetX = nextX;
                t.targetY = nextY;
                t.state = "moving";
            } else if (!isPlayer) {
    ...
    if (t.state === "moving") {
        addTrack(t);
        let dx = Math.abs(t.targetX - t.x);
        let dy = Math.abs(t.targetY - t.y);
        if (dx <= t.speed && dy <= t.speed) {
            t.x = t.targetX;
            t.y = t.targetY;
            t.state = "idle";
        } else {
            if (t.dir === "up") t.y -= t.speed;
            if (t.dir === "down") t.y += t.speed;
            if (t.dir === "left") t.x -= t.speed;
            if (t.dir === "right") t.x += t.speed;
        }
    }
}
...
function updateEngine() {
    ...
    for (let i = tracks.length - 1; i >= 0; i--) {
        if (now - tracks[i].time > 5000) tracks.splice(i, 1);
    }
    ...
    if (keys["KeyZ"] && now >= skills.shield.readyAt) {
        ...
        skills.shield.currentMax = 30000;
        skills.shield.readyAt = now + 30000;
        skills.shield.timerEnd = now + 5000;
        updateUI();
    }
    if (skills.shield.active && now > skills.shield.timerEnd) {
        skills.shield.active = false;
    }

    if (keys["KeyX"] && now >= skills.rapid.readyAt) {
        skills.rapid.active = true;
        skills.rapid.currentMax = 30000;
        skills.rapid.readyAt = now + 30000;
        skills.rapid.shotsLeft = 5;
        updateUI();
    }
    ...
    if (tank.isDead) {
        ...
        if (skills.rapid.active && now - window.lastShotTime >= 100 && skills.rapid.shotsLeft > 0) {
            if (player.shells[player.activeShell] > 0 || player.activeShell === 0) {
                ...
                player.globalStats.shellsUsed++;
                skills.rapid.shotsLeft--;
        ...
        } else if (skills.rapid.shotsLeft <= 0) {
            skills.rapid.active = false;
        }

        moveTankGridState(tank, true);

        if (keys["Space"] && !skills.rapid.active && 
            now - window.lastShotTime >= 1000 && 
            (player.activeShell === 0 || player.shells[player.activeShell] > 0)) {
            if (player.activeShell !== 0) player.shells[player.activeShell]--;
            player.globalStats.shellsUsed++;
            updateUI();
            window.lastShotTime = now;
    ...
    for (let i = bullets.length - 1; i >= 0; i--) {
        ...
        if (b.y < 0 || b.y > canvasElem.height || b.x < 0 || b.x > canvasElem.width) {
            hitObstacle = true;
        }

        if (!hitObstacle) {
            let col = Math.floor((b.x + b.w/2) / TILE);
            let row = Math.floor((b.y + b.h/2) / TILE);
            if (mapData[row] && mapData[row][col] && 
                (mapData[row][col].type === 1 || mapData[row][col].type === 2)) {
                if (mapData[row][col].type === 1) {
                    mapData[row][col].hp -= currentDmg;
                    if (b.isPlayer) {
                        addBattleProgress(0, Math.floor((currentDmg/100)*50), currentDmg);
                    }
                    if (mapData[row][col].hp <= 0) {
                        mapData[row][col].type = 0;
                        if (b.isPlayer) player.globalStats.envDestroyed++;
                    }
                }
                hitObstacle = true;
            }
        ...
        if (!hitObstacle && b.isPlayer) {
            for (let j = bots.length - 1; j >= 0; j--) {
                let bot = bots[j];
                if (!bot.isDead && bot.invulnTime < now &&
                    b.x > bot.x && b.x < bot.x + bot.w &&
                    b.y > bot.y && b.y < bot.y + bot.h) {
                    bot.hp -= currentDmg;
                    addBattleProgress(0, Math.floor((currentDmg/100)*50), currentDmg);
                    hitObstacle = true;
            ...
            if (!tank.isDead && tank.invulnTime < now &&
                b.x > tank.x && b.x < tank.x + tank.w &&
                b.y > tank.y && b.y < tank.y + tank.h) {
                hitObstacle = true;
                if (!skills.shield.active) {
                    player.hp -= 50;
                    if (player.hp <= 0) {
                        player.lives--;
                        player.globalStats.deaths++;
                        createBigExplosion(tank.x + tank.w/2, tank.y + tank.h/2);
                        craters.push({ x: tank.x, y: tank.y });
                        if (player.lives > 0) {
                            tank.isDead = true;
                            tank.respawnTime = now + 3000;
                        } else {
        ...
        if (hitObstacle) {
            explosions.push({ x: b.x + 2, y: b.y + 2, timer: 15, big: false });
            bullets.splice(i, 1);
        }
    }
}
...
function drawGrass(x, y) {
    ...
    ctx.fillStyle = "#2e662b";
    ctx.fillRect(x+4, y+4, 2, 4);
    ctx.fillRect(x+16, y+8, 2, 5);
    ctx.fillRect(x+10, y+16, 2, 4);
}
...
function drawCrater(x, y) {
    ...
    ctx.beginPath();
    ctx.arc(x + TILE/2, y + TILE/2, TILE * 0.7, 0, Math.PI * 2);
    ctx.fill();
...
function drawTracks() {
    ...
    tracks.forEach(tr => {
        ...
        ctx.save();
        ctx.translate(tr.x + TILE/2, tr.y + TILE/2);
        if (tr.dir === "up" || tr.dir === "down") {
            ctx.fillRect(-10, -12, 3, 24);
            ctx.fillRect(7, -12, 3, 24);
        } else {
...
function drawTank(t, isPlayer) {
    if (t.isDead) return;
    ctx.save();
    if (t.invulnTime > Date.now() && Math.floor(Date.now() / 150) % 2 === 0) {
        ctx.globalAlpha = 0.4;
    }
    ctx.translate(t.x + t.w/2, t.y + t.h/2);
    if (t.dir === "up") ctx.rotate(0);
    if (t.dir === "down") ctx.rotate(Math.PI);
    if (t.dir === "left") ctx.rotate(-Math.PI/2);
    if (t.dir === "right") ctx.rotate(Math.PI/2);
    ctx.fillStyle = "#222";
    ctx.fillRect(-12, -12, 5, 24);
    ctx.fillRect(7, -12, 5, 24);
    ctx.fillStyle = "#000";
    for (let i = -10; i < 12; i += 4) {
        ctx.fillRect(-12, i, 5, 1);
        ctx.fillRect(7, i, 5, 1);
    }
    ctx.fillStyle = t.color;
    ctx.fillRect(-7, -10, 14, 20);
    ctx.fillStyle = isPlayer ? "#004d00" : "#5c0000";
    ctx.beginPath();
    ...
    let curHp = isPlayer ? player.hp : t.hp;
    let maxHp = isPlayer ? player.maxHp : t.maxHp;
    ctx.fillStyle = "red";
    ctx.fillRect(t.x, t.y - 6, TILE, 4);
    ctx.fillStyle = "#00ff00";
    ctx.fillRect(t.x, t.y - 6, TILE * Math.max(0, curHp / maxHp), 4);
    ctx.strokeStyle = "#000";
    ctx.lineWidth = 1;
    ctx.strokeRect(t.x, t.y - 6, TILE, 4);
}
...
function drawEngine() {
    ...
    drawTank(tank, true);
    if (skills.shield.active && !tank.isDead) {
        ctx.strokeStyle = "rgba(0, 255, 255, 0.8)";
        ctx.lineWidth = 3;
        ctx.beginPath();
        ctx.arc(tank.x + 12, tank.y + 12, 16, 0, Math.PI * 2);
        ctx.stroke();
    }
    ...
    explosions.forEach(e => {
        if (e.big) {
            let r = (30 - e.timer) * 1.5;
            ctx.fillStyle = `rgba(255, ${Math.max(0, 150 - r*3)}, 0, ${e.timer/30})`;
            ctx.beginPath();
            ctx.arc(e.x, e.y, r, 0, Math.PI * 2);
            ctx.fill();
            ctx.fillStyle = `rgba(255, 255, 200, ${e.timer/30})`;
            ctx.beginPath();
